...
void loop() {
  if (digitalRead(tank_forward) && flag_tf == 0){
    mySerial.write('F');
    flag_tf = 1;
  }
  if (digitalRead(tank_forward) == LOW && flag_tf == 1){
    mySerial.write('S');
    flag_tf = 0;
  }
  
  if (digitalRead(gun_up) && flag_gu == 0){
    mySerial.write('f');
    flag_gu = 1;
  }
  if (digitalRead(gun_up) == LOW && flag_gu == 1){
    mySerial.write('S');
    flag_gu = 0;
  }
  
  if (digitalRead(gun_down) && flag_gd == 0){
    mySerial.write('b');
    flag_gd = 1;
  }
  if (digitalRead(gun_down) == LOW && flag_gd == 1){
    mySerial.write('S');
    flag_gd = 0;
  }
  /*if (digitalRead(tank_forward)){
    mySerial.write('F');
  }
  else if (digitalRead(tank_back)){
    mySerial.write('B');
  }
  else if (digitalRead(tank_right)){
    mySerial.write('R');
  }
  else if (digitalRead(tank_left)){
    mySerial.write('L');
  }
  else if (digitalRead(gun_up)){
    mySerial.write('f');
  }
  else if (digitalRead(gun_down)){
    mySerial.write('b');
  }
  else if (digitalRead(gun_right)){
    mySerial.write('r');
  }
  else if (digitalRead(gun_left)){
    mySerial.write('l');
  }*/
